...
export default function AvgCinematic({ isActive, onComplete }) {
  ...
  const renderBG = useCallback((elapsed, phase) => {
    ...
    // Nebula during comeback/restore
    if (phase === 'comeback' || phase === 'restore') {
      const grad = ctx.createRadialGradient(W/2,H/2,0,W/2,H/2,W*0.6)
      grad.addColorStop(0, `rgba(59,130,246,${0.06 + Math.sin(s.bgFrame * 0.02) * 0.02})`)
    ...
    // Comeback pulse on BG
    if (phase === 'comeback') {
      const pulse = Math.max(0, Math.sin((elapsed - 11) * 4) * 0.08)
      ctx.fillStyle = `rgba(59,130,246,${pulse})`
      ctx.fillRect(0, 0, W, H)
    }

    // Snap flash
    if (phase === 'snap' && elapsed < 1.1) {
      const flash = Math.max(0, 0.5 - (elapsed - 0.8) * 1.8)
  ...
  const renderParticles = useCallback((elapsed) => {
    ...
    // Darkness
    if (s.darkness > 0) {
      ctx.fillStyle = `rgba(0,0,0,${s.darkness * 0.6})`
      ctx.fillRect(0, 0, W, H)
    }
    ...
    s.ashParticles.forEach(p => {
      ...
      ctx.rotate(p.rot)
      ctx.globalAlpha = Math.min(p.life, 0.9) * p.z
    ...
    if (s.phase === 'restore') {
      ...
      s.rebuildParticles.forEach(p => {
        ...
        const trailLife = Math.max(0, p.life - 0.1)
      ...
      const burstAlpha = Math.sin(burstProgress * Math.PI) * 0.3
      if (burstAlpha > 0) {
        const gr = ctx.createRadialGradient(W/2,H/2,0, W/2,H/2, 100*burstProgress+50)
        gr.addColorStop(0, `rgba(255,255,255,${burstAlpha})`)
        gr.addColorStop(0.5, `rgba(96,165,250,${burstAlpha * 0.5})`)
